...
| `akqa-muted` | `#999999` | `#999999` | Captions (alias of dusty) |
| `akqa-border` | `#E5E5E5` | `#404040` | Borders, dividers, inputs |
| `akqa-white` | `#FFFFFF` | — | Card surfaces on light; text on dove |
| `surface-elevated` (derived) | `#FFFFFF` | `#2A2A2A` | Cards, columns (`dark:bg-[#2a2a2a]`) |
| `surface-input` (derived) | `#FFFFFF` | `#1A1A1A` | Textareas (`dark:bg-[#1a1a1a]`) |
| `overlay-scrim` | `rgba(0,0,0,0.5)` | same | Modal backdrop |
| `destructive` | `#DC2626` | `#F87171` | Delete hover (`red-600` / `red-400`) |
...
| Icon button (card) | 24×24px | Edit, delete, Done |
| Vote control | 20×20px | +/- |
...
| **Theme toggle** | Light / Dark / System | 36×36 target; border `akqa-border` |
...
| **Button / Destructive (text)** | Hover | Muted → red |
...
- **Touch targets**: Keep card icon buttons at least **24px**.
